...
const ExprStart = /^\s*\[\s*/;
const ExprEnd = /^\s*\]\s*/;
function Option(...rest: any[]) {
  return Node(
    All(OptionStart, All(OptionText, Optional(SimpleExpression))),
    ([optionText, ...otherOptionStuff]) =>
      echo({
        type: "Option",
        optionText,
        otherOptionStuff,
...
function Options(...rest: any[]) {
  return Node(Star(Option), options => echo(options))(...rest);
}
function SimpleExpression(...rest: any[]) {
  return Node(All(ExprStart, Hashtags, Options, ExprEnd), ([hashes, options]) =>
    echo({
      type: "SimExpr",
      hashes,
      options,
